...
- **Local storage**: FrameComment uses TUS resumable uploads — the upload should resume after a network interruption. Check available disk space and directory permissions.
...
- The bucket must allow `PUT` requests from your app origin. Check your CORS `AllowedOrigins` matches the URL you use to access FrameComment (including protocol and port).
...
**502 Bad Gateway**
- Ensure the FrameComment container is running and healthy.
