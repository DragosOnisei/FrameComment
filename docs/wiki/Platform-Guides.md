...
```bash
curl -O https://raw.githubusercontent.com/DragosOnisei/FrameComment/main/docker-compose.unraid.yml
curl -O https://raw.githubusercontent.com/DragosOnisei/FrameComment/main/.env.example
```
...
**Quick install:** Use the TrueNAS Apps catalog (Apps → Discover Apps → FrameComment).

**Manual install:**
1. Create datasets: `postgres`, `redis`, `uploads`.
2. Download config:
```bash
curl -O https://raw.githubusercontent.com/DragosOnisei/FrameComment/main/docker-compose.truenas.yml
```
3. Generate secrets (see Installation).
