...
| `POSTGRES_USER` | Yes | PostgreSQL username | `framecomment` | `framecomment` |
| `POSTGRES_PASSWORD` | Yes | PostgreSQL password (hex only) | - | Generated with `openssl rand -hex 32` |
| `POSTGRES_DB` | Yes | PostgreSQL database name | `framecomment` | `framecomment` |
...
| `S3_BUCKET` | When `STORAGE_PROVIDER=s3` | Bucket name (must already exist) | — | `framecomment` |
...
**Tested with:** MinIO AIStor (self-hosted Docker container). Other S3-compatible stores (AWS S3, Cloudflare R2, Backblaze B2, Garage, etc.) should work but have not been tested — please [open an issue](https://github.com/DragosOnisei/FrameComment/issues) if you run into any problems.

**How it works:**
- **Uploads** — the browser requests presigned part URLs from FrameComment, then PUTs each chunk straight to the store. Node.js never touches the file bytes. This applies to all uploads: videos, comment attachments, and client file submissions (reverse share).
...
**Requirements:**
1. Create the bucket before starting FrameComment.
...
**MinIO AIStor** — uses path-style addressing, which FrameComment enables automatically when `S3_ENDPOINT` is set. Set `S3_ENDPOINT` to your MinIO hostname (e.g. `http://minio:9000`).
...
mc admin config set myminio/ api cors_allow_origin=https://framecomment.example.com
...
[
  {
    "AllowedOrigins": ["https://framecomment.example.com"],
...
`CPU_THREADS` controls how FrameComment allocates CPU resources for video encoding. When set, it overrides the auto-detected CPU count and determines three things:
