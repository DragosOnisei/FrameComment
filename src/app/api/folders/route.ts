...
import {
  copyableCommentCountsByVideo,
  enrichVideosForAdmin,
} from '@/lib/folder-video-enrichment'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (parentFolderId === null) {
      ...
      // 7.5.0: same copy-carry count the folder route attaches — the root
      // grid's context menu makes the same "Copy N comments" promise.
      const copyableByVideo = await copyableCommentCountsByVideo(
        rootVideos.map((v: any) => v.id),
      )
      const videosWithCopyable = enrichedVideos.map((v: any) => ({
        ...v,
