...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ error: videoMessages.videoNameRequired || 'Video name is required' }, { status: 400 })
    ...
    // Resolve a unique name in this (project, folder) scope. Sequential
    // uploads with the same base name get " (2)", " (3)", … Simultaneous
    // uploads are de-duped CLIENT-SIDE within the batch before they're
    // sent (see VideoUploadModal), which handles the multi-select case
    // without a DB lock. (An earlier server-side advisory-lock attempt
    ...
    try {
      video = await prisma.video.create({
        data: { ...baseCreate, createdById: admin.id },
      })
    } catch {
