...
export async function sendEmail({
  ...
  try {
    ...
    const fromAddress = settings.smtpFromAddress || settings.smtpUsername || 'noreply@framecomment.local'
    const companyName = sanitizeEmailHeader(settings.companyName || 'FrameComment')
...
export async function sendNewVersionEmail({
  ...
  const settings = await getEmailSettings()
  const companyName = settings.companyName || 'FrameComment'
...
export async function testEmailConnection(testEmail: string, customConfig?: any) {
  ...
  const smtpTestMessages = emailMessages?.smtpTest || {}
    ...
    // Send test email
    const html = renderEmailShell({
      companyName: settings.companyName || 'FrameComment',
