...
export function ShareTutorial({
  ...
  const buildSteps = useCallback((): DriveStep[] => {
    ...
    steps.push({
      popover: {
        title: t('doneTitle'),
        description: `${t('doneDescription')} <a href="https://github.com/DragosOnisei/FrameComment/wiki/Client-Guide" target="_blank" rel="noopener noreferrer" style="text-decoration:underline;opacity:0.8">${t('doneLearnMore')}</a>`,
      },
    })
